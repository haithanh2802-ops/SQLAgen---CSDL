...
NO_PARAMETERS = {"no_parameters": True}


def _execute_literal_sql(connection: Any, sql: str) -> Any:
    """Execute validated SQL without DB-API percent-style interpolation."""
    return connection.exec_driver_sql(sql, execution_options=NO_PARAMETERS)
    ...
    def preview_write(self, sql: str) -> ImpactPreview:
        with self.write_engine.connect() as connection:
            ...
            try:
                plan = [
                    dict(row)
                    for row in _execute_literal_sql(connection, f"EXPLAIN {sql}").mappings()
                ]
            finally:
                connection.rollback()
...
def format_schema_catalog(catalog: dict[str, list[dict[str, Any]]]) -> str:
    ...
    for table_name, columns in catalog.items():
        ...
        for column in columns:
            ...
            if column.get("collation_name"):
                attributes.append(f"COLLATE {column['collation_name']}")
