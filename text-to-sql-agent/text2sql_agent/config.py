...
class Settings:
    ...
    memory_persistent_threshold: float
    ...
    def from_env(cls, project_root: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            memory_enabled=boolean("MEMORY_ENABLED", True),
            memory_directory=local_path("MEMORY_DIRECTORY", ".local/qdrant-memory"),
            memory_collection=os.getenv("MEMORY_COLLECTION", "conversation_memory"),
            memory_user_id=os.getenv("MEMORY_USER_ID", "local-user").strip(),
            memory_project_id=os.getenv("MEMORY_PROJECT_ID", "olist-analytics").strip(),
            memory_persistent_threshold=float(
                os.getenv("MEMORY_PERSISTENT_THRESHOLD", "0.75")
            ),
    ...
    def _validate(self) -> None:
        ...
        for name in ("memory_persistent_threshold", "memory_min_similarity"):
            value = getattr(self, name)
            if not 0 <= value <= 1:
                raise ValueError(f"{name} must be between zero and one.")
        if not self.memory_user_id or not self.memory_project_id:
            raise ValueError("MEMORY_USER_ID and MEMORY_PROJECT_ID cannot be blank.")
