...
AGENT_CACHE_VERSION = "memory-policy-v1"
...
def service_status(model_name: str) -> dict[str, Any]:
    ...
    try:
        status["saved_memories"] = agent.memory.count(
            user_id=agent.settings.memory_user_id
        )
    except Exception as exc:  # noqa: BLE001 - health check must not crash the UI
...
def forget_saved_memory(model_name: str) -> None:
    agent = get_agent(model_name, AGENT_CACHE_VERSION)
    removed = agent.memory.clear_user(user_id=agent.settings.memory_user_id)
    service_status.clear()
    st.session_state.session_notice = f"Deleted {removed} saved memories."
...
def confirm_end_session() -> None:
    st.write(
        ...
        "Saved long-term memories and database changes are not removed."
    )
...
@st.dialog("Forget saved memory?", icon=":material/memory:")
def confirm_forget_saved_memory(model_name: str) -> None:
    st.write(
        "This permanently removes all long-term memories for the configured local user. "
        "It does not delete business documents, database rows, or the current chat."
    )
    with st.container(horizontal=True, horizontal_alignment="right"):
        if st.button("Keep memory", key="keep_saved_memory"):
            st.rerun()
        st.button(
            "Forget saved memory",
            type="primary",
            icon=":material/delete_forever:",
            key="confirm_forget_saved_memory",
            on_click=forget_saved_memory,
            args=(model_name,),
        )
...
def render_evidence(response: AgentResponse) -> None:
    ...
    if response.retrieved_memories:
        st.markdown("**Relevant saved memory**")
...
def render_non_result(response: AgentResponse, turn_id: str) -> None:
    if response.kind == "memory":
        st.info(
            response.summary.answer if response.summary else "Memory request completed.",
...
st.session_state.setdefault("memory_session_id", uuid.uuid4().hex)
...
with st.sidebar:
    ...
    service_errors = (
        "database_error" in status
        or "knowledge_error" in status
        or "memory_error" in status
    )
    if service_errors:
    ...
    with st.expander("Diagnostics", icon=":material/monitor_heart:"):
        ...
        if "memory_error" in status:
            st.warning("Long-term memory unavailable")
        elif configured_settings.memory_enabled:
            st.caption(f"Saved memory: {status['saved_memories']:,} items")
        else:
            st.caption("Saved memory: Disabled")
    ...
    if st.button(
        "Forget saved memory",
        icon=":material/memory:",
        disabled=not configured_settings.memory_enabled
        or not status.get("saved_memories", 0),
        key="forget_saved_memory",
    ):
        confirm_forget_saved_memory(selected_model)
    st.caption(
        "Recent chat is temporary. Approved high-value preferences and decisions may be saved "
        "separately as long-term memory."
    )
...
if question_to_run:
    recent_messages = recent_chat_messages(st.session_state.conversation)
    turn = {
        "id": uuid.uuid4().hex,
    ...
    with st.chat_message("assistant", avatar=":material/analytics:"):
        ...
        response = get_agent(selected_model, AGENT_CACHE_VERSION).ask(
            question_to_run,
            user_id=configured_settings.memory_user_id,
            project_id=configured_settings.memory_project_id,
