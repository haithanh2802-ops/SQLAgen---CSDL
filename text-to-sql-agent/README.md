...
Questions and answers remain visible for the current browser-tab session. Use
**End session** in the sidebar to confirm deletion of the complete conversation.
Chat content is held only in Streamlit session state and is not persisted as
chat history. Privacy-conscious audit events continue to record execution
metadata without storing the question or full result set.
